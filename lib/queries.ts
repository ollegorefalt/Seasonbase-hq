...
export async function getOverviewMetrics(): Promise<OverviewMetrics> {
  ...
  const [
    ...
    supabase.from('employers').select('*', { count: 'exact', head: true }).in('status', ['lead', 'contacted', 'meeting booked', 'follow-up', 'interested']),    supabase.from('meetings').select('*', { count: 'exact', head: true }).gte('meeting_date', monthStart),
  ]);
